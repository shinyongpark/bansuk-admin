...
const ProductsByCategory = () => {
  ...
  const fetchTotalPrimeCost = async () => {
    try {
      const response = await axios.get('http://3.34.143.58:80/get-total-prime-cost');
  ...
  const fetchProducts = async (categoryId) => {
    try {
      const response = await axios.get(`http://3.34.143.58:80/get-products?category=${categoryId}`);
  ...
  const handleCostUpdate = async (id, e) => {
    ...
    try {
      ...
      const response = await axios.post('http://3.34.143.58:80/update-product-cost', {
        id,
